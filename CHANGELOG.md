...
Fixed
withdraw_funds now names the campaign creator alongside the campaign id in reserve_withheld, matching the reserve_released event emitted by withdraw_reserve. Both reserve events now carry the campaign id and creator in their topics and the exact reserve amount as data, so indexers can audit the withholding and payout of a campaign's vested reserve without diffing token balances (#852).
cancel_campaign and admin_cancel_campaign now clear the campaign's stored top-contributor marker, so get_campaign_stats no longer reports a winner for a terminal campaign whose contributions are refundable; the query also ignores a stale marker left behind on campaigns cancelled by an older version of the contract (#863).
