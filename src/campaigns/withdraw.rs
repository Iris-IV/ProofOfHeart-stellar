...
pub(crate) fn withdraw_funds(env: &Env, campaign_id: u32) -> Result<(), Error> {
    ...
    let withdraw_basis = campaign.effective_amount_raised;
    let fee_amount = withdraw_basis
        .checked_mul(platform_fee as i128)
        .and_then(|n| n.checked_add(9999))
        .ok_or(Error::Overflow)?
        / 10000;
    let total_after_fee = withdraw_basis - fee_amount;
    ...
    set_total_raised_global(
        env,
        total_raised
            .checked_sub(withdraw_basis - reserve_amount)
            .ok_or(Error::Overflow)?,
    );
