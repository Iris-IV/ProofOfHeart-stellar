...
pub(crate) fn update_campaign(
    ...
    if campaign.amount_raised > 0 {
        return Errn(Error::ValidationFailed);
    }
...
pub(crate) fn update_campaign_description(
    ...
    //
    // `update_campaig`n freezes title and description once verified (#416),
    ...
    //
    // Revoking rather than rejecting keeps the edit available — a creator can still correct their copy — at the cost of re-verification. Note this
...
pub(crate) fn extend_campaign_deadline(
    ...
    if total_duration_days > category_cap {
        return Errn(Error::InvalidDuration);
    }
