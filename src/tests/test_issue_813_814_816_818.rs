...
fn test_cancel_campaign_allows_accept_token_update_after_all_refunds_claimed() {
    ...
    let id = make_campaign(&env, &client, &creator, 1_000, 30, Category::Educator, 1);
    client.contribute(&id, &contributor, &500);
    client.cancel_campaign(&id);
...
fn test_claim_refund_does_not_double_decrement_after_creator_cancel() {
    ...
    let id = make_campaign(&env, &client, &creator, 1_000, 30, Category::Learner, 2);
...
fn test_admin_cancel_decrements_total_raised_global() {
    ...
    let id = make_campaign(&env, &client, &creator, goal, 30, Category::Educator, 3);
...
fn test_failed_funding_claim_refund_still_decrements_total_raised_global() {
    ...
    let id = make_campaign(&env, &client, &creator, 1_000, 30, Category::Learner, 4);
...
fn test_verify_campaigns_partial_failure_returns_both_vecs() {
    ...
    let good = make_campaign(&env, &client, &creator, 100, 30, Category::Learner, 5);
    let bad_id: u32 = 9999;
...
fn test_verify_campaigns_all_succeed_returns_empty_failed_vec() {
    ...
    let id1 = make_campaign(&env, &client, &creator, 100, 30, Category::Learner, 6);
    let id2 = make_campaign(&env, &client, &creator, 200, 30, Category::Educator, 7);
...
fn test_extend_deadline_blocked_when_new_total_exceeds_category_cap() {
    ...
    let id = make_campaign(&env, &client, &creator, 100, 60, Category::Learner, 8);
...
fn test_extend_deadline_allowed_within_category_cap() {
    ...
    let id = make_campaign(&env, &client, &creator, 100, 60, Category::Learner, 9);
...
fn test_burst_guard_block_counts_are_per_campaign_not_global() {
    ...
    let id_a = make_campaign(&env, &client, &creator, 1_000, 30, Category::Learner, 10);
    let id_b = make_campaign(&env, &client, &creator, 1_000, 30, Category::Educator, 11);
